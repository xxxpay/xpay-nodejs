...
var WxOAuth = module.exports = {
        ...
        res.on('end', function () {
          try {
            ...
            if (responseJson.hasOwnProperty('errcode')
              ...
              return reject(new Error.XPayChannelError({
                detail: responseJson,
  ...
  getSignature: function (payment, jsapi_ticket, url) {
    if (!payment.hasOwnProperty('credential') || !payment['credential'].hasOwnProperty('wx_pub')) {
      return null;
    }
    var credential = payment['credential']['wx_pub'];
    var arrayToSign = [

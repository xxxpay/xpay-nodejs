...
xpay.balanceTransfers.create(APP_ID, params_create, function(err, balanceTransfer) {
  if (err != null){
    console.log('xpay.balanceTransfers.list fail:', err);
  }
  // YOUR CODE
});
```
...
### 查询单个余额转账
``` js
xpay.balanceTransfers.retrieve(APP_ID,
  '660170829413280122880001', // BalanceTransfers ID
  function(err, balanceTransfer) {
    if (err != null){
      console.log('xpay.balanceTransfers.retrieve fail:', err);
    }
    // YOUR CODE
});
...
### 查询余额转账列表
``` js
xpay.balanceTransfers.list(APP_ID, {'page': 1, 'per_page': 3}, function(err, data) {
  if (err != null){
    console.log('xpay.balanceTransfers.list fail:', err);
  }
  // YOUR CODE

...
xpay.recharge.create(APP_ID, params, function(err, recharge) {
  if (err!=null){
    console.log('xpay.recharge.create fail:',err)
  }
  // YOUR CODE
});
```
...
### 查询单个 recharge
``` js
xpay.recharge.retrieve(APP_ID,
  '220170829596751616000001', // Recharge ID
  function(err, recharge) {
    if (err!=null){
      console.log('xpay.recharge.retrieve fail:',err)
    }
    // YOUR CODE
});
```
...
### 查询 recharge 列表
``` js
xpay.recharge.list(APP_ID, {'page': 1, 'per_page': 3}, function(err, recharges) {
  if (err!=null){
    console.log('xpay.recharge.list fail:',err)
  }
  // YOUR CODE
});
```

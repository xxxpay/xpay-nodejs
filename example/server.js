...
// 接入支付流程参考开发者中心：https://pay.lucfish.com/docs/server/payment ，文档可筛选后端语言和接入渠道。
...
var createPayment = function (channel, amount, client_ip, open_id, cb) {
  // 以下 channel 仅为部分需要 extra 参数的示例，详见 https://pay.lucfish.com/document/api#api-c-new
  var extra = {};
  switch (channel) {
    case 'alipay_wap':
      extra = {
  ...
  xpay.payments.create({
    ...
    channel: channel, // 支付使用的第三方支付渠道取值，请参考：https://pay.lucfish.com/api#api-c-new
...
http.createServer(function (req, res) {
  ...
  xpay.parseHeaders(req.headers); // 把从客户端传上来的 Headers 传到这里

  // 设置请求签名密钥，密钥对需要你自己用 openssl 工具生成，如何生成可以参考帮助中心：https://help.pay.lucfish.com/article/123161；
  // 生成密钥后，需要在代码中设置请求签名的私钥(rsa_private_key.pem)；
  // 然后登录 [Dashboard](https://dashboard.pay.lucfish.com)->点击右上角公司名称->开发信息->商户公钥（用于商户身份验证）
  // 将你的公钥复制粘贴进去并且保存->先启用 Test 模式进行测试->测试通过后启用 Live 模式

  // 设置你的私钥路径，用于请求的签名，对应的公钥请填写到 XPay 管理平台
  ...
  req.addListener("end", function () {
    ...
    switch (req.url) {
      case "/pay":
        ...
        createPayment(channel, amount, client_ip, open_id, function (err, payment) {
          if (payment != null) {
            return resp(payment);
          }
          return resp({
